...
const mapSelectorsToDispatch = (selectors, state, statePath, opts) => ({
  ...get(state, statePath),
  ...mapValues(selectors, (selector, selectorKey) => {
...
const mapActionToDispatch = (actions, store, dispatch, opts, actionPrefix = '') =>
  mapValues(actions, (action, actionKey) => {
...
export default stores => {
  const stateMappers = (state, opts) =>
    mapValues(stores, (store, storeKey) => mapSelectorsToDispatch(store.selectors, state, storeKey, opts));
  const dispatchMappers = (dispatch, opts) =>
    mapValues(stores, store => mapActionToDispatch(store.actions, store, dispatch, opts));
